...
class CelerSightRemoteAnnotationAdminTest(unittest.TestCase):
    ...
    @parameterized.expand(
        [
            (
                ["fixtures//pattern_single_channel_per_folder"], # imported image paths , can be a directory
                {
                    "aup-" : 3,
    ...
    def test_import_pattern(self, image_paths , expected_loaded_image_amount , expected_channels, action):
        ...
        for treatment_name , expected_amount in expected_loaded_image_amount.items():
            ...
            assert len(app.DH.BLobj.groups[current_group].conds[treatment_name].images) == expected_amount
            for image_object in app.DH.BLobj.groups[current_group].conds[treatment_name].images:
